...
Before packaging, build the core XCFramework from source revision `b39dbdc2877a60c6666697cc0817f31225496cb2` and run all commands in `docs/testing.md`. CI pins that immutable source revision; the typed host-secret head still requires a successful hosted macOS run before it can be treated as verified. A release must use a verified immutable artifact and checksum from the central release manifest.
