...
- The workflow pins reviewed core ABI 1 source revision `115535c76d804020f045708867af7798b8d0294a`; it never consumes moving `main`. The macOS client workflow has not run, so its XCFramework build, wrapper tests, client linkage, and application behavior remain unverified on macOS.
...
- GitHub API reads confirmed that the pinned checkout and Rust-toolchain Action commits and the pinned project, core, and localization repository commits are reachable. The core header and generated wrapper at `115535c76d804020f045708867af7798b8d0294a` declare ABI major 1, protocol 1, engine-bound buffer release, and resource-exhaustion result mapping. Core Native SDK run `29557243604` passed its Linux, Windows, Android, and Apple jobs. Its Apple artifact `8397926235` verified the exact source revision, ABI 1, protocol 1, prerelease status, `SHA256SUMS`, and SwiftPM checksum. This is dependency evidence only; it does not verify macOS client linkage or behavior.
